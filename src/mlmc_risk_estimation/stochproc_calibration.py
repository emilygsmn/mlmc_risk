...
def _get_return_time_incr(ret_length):
    if ret_length == "week":
        ...
        raise ValueError(f"Unknown return length '{ret_length}'. Only weekly and quarterly are implemented.")
...
def _get_num_returns_pa(ret_length):
    if ret_length == "week":
        ...
        ValueError(f"Unknown return length '{ret_length}'. Only weekly and quarterly are implemented.")
...
def calibrate_models(mkt_data, instr_info, param_config):
    ...
    for rf in rfs:
        ...
        volas.loc["sigma", rf] = _get_empirical_vola(time_series=np.array(mkt_data[rf]),
